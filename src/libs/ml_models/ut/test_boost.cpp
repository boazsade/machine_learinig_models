...
BOOST_AUTO_TEST_CASE(empty_data_boost)
{
    mlmodels::boosting model;
    try {
    auto m = model.train(mlmodels::training_data{}, 
            mlmodels::class_data{},
            mlmodels::boosting::args{}
        );
    ...
    BOOST_TEST(out.empty(), "there should not be any data from the test");
        ...
        BOOST_TEST(false, "unexpected exception: "<<e.what());
    }
}

BOOST_AUTO_TEST_CASE(test_model_create_boost)
{
    ...
    std::cout<<"model prediction for boosting\n";
    for (auto f : p) {
        std::cout<<"["<<f<<"]";
    }
    std::cout<<std::endl;
