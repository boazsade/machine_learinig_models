...
task_result run_task(train_step ts, const classifier::read_result& data,
            lookup::lookuptable_transform tras_table);
...
task_result run_task(train_step ts, const classifier::read_result& data,
            lookup::lookuptable_transform tras_table)
{
    return std::async(std::launch::async, [ts, &data, tras_table]() {
            return model2experiment(std::move(ts), data, std::move(tras_table));
        }
    );
}
            ...
            lookup::lookuptable_transform tras_table)
{
    ...
    auto m_name = name(ts);
    LOG_DEBUG<<"running model experiment for "<<m_name<<" with data input - ";
    for (const auto& col : data) {
        LOG_DEBUG<<col;
    }
    // for a single train/test entry we have to 
    // 1. train
    auto test_s = train(ts, data, tras_table);
    // 2. test
    auto test_res = test(test_s, data, tras_table);
...
experiment::result_type run_experiments(train_data&& td,
        const classifier::read_result& data,
        lookup::map_value_op transformer)
{
    ...
    tasks_output to; 
    lookup::lookuptable_transform lt{preform_action(transformer, data)}; 
    boost::transform(td, std::back_inserter(to),
                    [&data, &lt](auto ts) { 
                        return run_task(std::move(ts), data, lt);
                }
            );   
    ...
    experiment::run(config&& conf) const
{
    ...
    return run_experiments(std::move(td), all_data, 
            lookup::map_value_op{conf.target_mapper});
}
