...
from tp_stats import columnStats
...
def format_output(dataframe):
    ...
    if df.count() < dl:
        df = dataframe
    # make sure not to include index column in samples 
    df = df.drop('index')
    samples = df.head(dl)
    return {'rows': samples}


def make_stats(dataframe, schema, cstats):
    results = []
    for col in schema:
        s = columnStats(colSchema=col, df=dataframe, colStats=cstats)
        if s is not None:
            results.append(s)
    return results
...
def do_export(dataframe, settings, schema):
    try:
        return export2file(df=dataframe, conf=settings['export'], schema=schema)
...
def process_command(command):
    ...
    spark = SparkSession.builder.appName("tx_dataprep").master("local[16]").getOrCreate()
    ...
    # df, stats = do_convertion(dataframe=df, schema=command['schema']['cols'])
    newSchema = []
    ...
    if len(newSchema) != 0:
        output.update({'cols': make_stats(dataframe=df, schema=newSchema, cstats=stats)})
    else:
        logging.warning("we don't have a schema to return back - this is a serious issue!!")
    colmap = do_export(dataframe=df, settings=command, schema=newSchema)
    if colmap is not None:
        if len(colmap) != 0:
            logging.info("saving mapping to the message")
            output.update({'lookup': colmap})
...
def main(argv=sys.argv[1:]):
    if len(argv) != 1:
        ...
        return -1
    try:
        init_log(name="data_prep")
        logging.warning(
            "\n========================\n========================\n\tstarting data prep\n========================\n========================\n")
    except Exception as e:
        logging.fatal("critical error: {}".format(e))
